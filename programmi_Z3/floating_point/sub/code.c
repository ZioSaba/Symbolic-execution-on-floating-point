...
void eval()
{
    ...
    Z3_ast c2 = Z3_mk_fpa_lt(ctx, y, zero);
    /****************************/
    ...
    //Solve: -/y with x<y & y<0

    /* find model for the constraints above */
    if (Z3_solver_check(ctx, s) == Z3_L_TRUE) {
        
        Z3_symbol sottrazione_sym = Z3_mk_string_symbol(ctx, "x_sub_y");
        Z3_ast x_sub_y = Z3_mk_const(ctx, sottrazione_sym, FP_sort);
        Z3_ast eq = Z3_mk_fpa_sub(ctx, rm, x, y);
