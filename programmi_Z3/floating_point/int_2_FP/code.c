...
*/
void conversion_SInt_FP()
{
    Z3_context ctx = mk_context();
    Z3_solver s = mk_solver(ctx);

    Z3_sort bv_sort = Z3_mk_bv_sort(ctx, 32);
    Z3_ast two_INT = Z3_mk_numeral(ctx, "2", bv_sort);

    Z3_ast rm = Z3_mk_fpa_rtz(ctx);
    Z3_sort fpa_sort = Z3_mk_fpa_sort(ctx, 8, 24);
    Z3_ast two_FP = Z3_mk_fpa_to_fp_signed(ctx, rm, two_INT, fpa_sort);

    Z3_ast reale = Z3_mk_fpa_to_real(ctx, two_FP);
    

    printf("bv_sort -> %s\n", Z3_ast_to_string(ctx, two_INT));
    ...
    if (Z3_solver_check(ctx, s) == Z3_L_TRUE) {
        ...
        Z3_ast v;
        if (Z3_model_eval(ctx, m, two_FP, 1, &v)) {
    ...
    if (Z3_solver_check(ctx, s) == Z3_L_TRUE) {
        ...
        Z3_ast v;
        if (Z3_model_eval(ctx, m, reale, 1, &v)) {
            printf("\nMODEL:\nreale -> %s\n", Z3_ast_to_string(ctx, v));
            if (m) Z3_model_dec_ref(ctx, m);
        }
        else {
    ...
    //printf("fpa_sort -> %s\n", Z3_ast_to_string(ctx, two_FP));
    //printf("num reale -> %s\n", Z3_ast_to_string(ctx, reale));
